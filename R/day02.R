...
#'   `f02b(x)` returns ....
#' @importFrom dplyr group_by mutate summarise left_join filter pull
#' @importFrom tidyr unnest_longer separate
...
f02a <- function(x) {
  ...
  res <- df %>%
    ...
    dplyr::summarise(game = sum(as.numeric(game))) %>%
    dplyr::pull(game)
  return(res)
}
...
example_data_02 <- function(example = 1) {
  tmp<-'Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green'
